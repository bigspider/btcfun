...
class TreeNode:
    ...
    def __init__(self, left_child, right_child):
        ...
        # Cached mapping from a script present in the Taproot tree to its merkle proof.
        self.leaves = self._compute_leaves()

    def __repr__(self):
        return f"{{{self.left_child},{self.right_child}}}"

    def _compute_leaves(self, merkle_proof=[]):
        """Internal method to compute the leaf-to-merkle-proof mapping when instanciated."""
        ...
        if isinstance(self.left_child, Node):
            return {
                ...
                **self.right_child._compute_leaves(
        ...
        if isinstance(self.right_child, Node):
            return {
                ...
                **self.left_child._compute_leaves(
