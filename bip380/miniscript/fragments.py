...
class RequiredContext(Enum):
    """The context in which this Miniscript must be used."""
...
class Node:
    ...
    exec_info = None
    # Some fragments are only valid in some context. For instance multi_a may only be used
    # under Tapscript and multi() may only be used under P2WSH. Most fragments can be used
    # in any context.
    req_context = RequiredContext.NONE
    ...
    def _set_req_context(self):
        """Set the required context of this fragment based on its sub-fragments."""

        # Must not be set already
        assert self.req_context == RequiredContext.NONE

        # Set our required context based on our child, checking for conflicts
        for sub in self.subs:
            if sub.req_context == RequiredContext.P2WSH:
                if self.req_context == RequiredContext.TAPSCRIPT:
                    raise MiniscriptNodeCreationError(
                        "Uses both fragments reserved for P2WSH and Tapscript"
                    )
                self.req_context = RequiredContext.P2WSH
            if sub.req_context == RequiredContext.TAPSCRIPT:
                if self.req_context == RequiredContext.P2WSH:
                    raise MiniscriptNodeCreationError(
                        "Uses both fragments reserved for P2WSH and Tapscript"
                    )
                self.req_context = RequiredContext.TAPSCRIPT
...
class Multi(Node):
    def __init__(self, k, keys):
        ...
        # CHECKMULTISIG isn't available under Tapscript
        self.req_context = RequiredContext.P2WSH
...
class MultiA(Node):
    def __init__(self, k, keys):
        ...
        # CHECKSIGADD isn't available under P2WSH
        self.req_context = RequiredContext.TAPSCRIPT
