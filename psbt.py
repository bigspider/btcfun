...
class PartiallySignedInput:
        ...
        while True:
            ...
            if key_type == PartiallySignedInput.PSBT_IN_NON_WITNESS_UTXO:
                ...
                if key in key_lookup:
                    ...
                    raise PSBTSerializationError("Input Musig2 aggregate x-only pubkey is not 32 bytes")
                ...
                for i in range(0, len(pubkeys_cat), 33):
                    pubkeys.append(pubkeys_cat[33*i : 33*(i+1)])

                self.musig2_participant_pubkeys[key] = pubkeys
                ...
                elif len(key) not in [33 + 32, 33 + 32 + 32]:
                    raise PSBTSerializationError("Invalid key length for Musig2 public nonce")

                providing_pubkey = key[:33]
                target_participant_xonly_pubkey = key[33:33+32]
                tapleaf_hash = None if len(key) == 33 + 32 else key[33+32:]
                ...
                self.musig2_pub_nonces[(providing_pubkey, target_participant_xonly_pubkey, tapleaf_hash)] = public_nonces
    ...
    def serialize(self) -> bytes:
        ...
        for xonly, pubkeys in self.musig2_participant_pubkeys.items():
            r += ser_string(ser_compact_size(PartiallySignedInput.PSBT_IN_MUSIG2_PARTICIPANT_PUBKEYS) + xonly)
            r += ser_string(b''.join(pubkeys))

        for (pk, xonly, hash), pubnonce in self.musig2_pub_nonces.items():
            key_value = pk + xonly + (b'' if hash is None else hash)
            r += ser_string(ser_compact_size(PartiallySignedInput.PSBT_IN_MUSIG2_PARTICIPANT_PUBKEYS) + key_value)
            r += ser_string(pubnonce)

        for (pk, xonly, hash), partial_sig in self.partial_sigs.items():
            key_value = pk + xonly + (b'' if hash is None else hash)
            r += ser_string(ser_compact_size(PartiallySignedInput.PSBT_IN_MUSIG2_PARTICIPANT_PUBKEYS) + key_value)
...
class PartiallySignedOutput:
        ...
        while True:
            ...
            if key_type == PartiallySignedOutput.PSBT_OUT_REDEEM_SCRIPT:
                ...
                if key in key_lookup:
                    ...
                    raise PSBTSerializationError("Output Musig2 aggregate x-only pubkey is not 32 bytes")
    ...
    def serialize(self) -> bytes:
        ...
        for xonly, pubkeys in self.musig2_participant_pubkeys.items():
            r += ser_string(ser_compact_size(PartiallySignedOutput.PSBT_OUT_MUSIG2_PARTICIPANT_PUBKEYS) + xonly)
            r += ser_string(b''.join(pubkeys))
